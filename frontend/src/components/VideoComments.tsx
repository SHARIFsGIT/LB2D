...
const VideoComments: React.FC<VideoCommentsProps> = ({ videoId, courseId }) => {
  ...
  const handleUpdateReply = async (replyId: string) => {
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/video-comments/${replyId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleMarkResolved = async (commentId: string, isResolved: boolean) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/video-comments/${commentId}/resolve`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
