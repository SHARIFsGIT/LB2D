  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      if (!allowedTypes.includes(selectedFile.type)) {
        setError('File type not supported. Please upload PDF, DOC, DOCX, MP3, WAV, OGG, JPG, PNG, GIF, or TXT files.');
        return;
      }
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
        <p className="text-xs text-blue-700">
          <strong>Note:</strong> Resources will be saved as drafts and need to be submitted for admin approval before students can access them.
        </p>
      </div>
    </div>
  );
