              ...
              {formData.password && (
                <div className="mt-2">
                  ...
                  <div className="mt-3 p-3 bg-gray-50 bg-opacity-80 backdrop-blur-sm rounded-lg border border-gray-200">
                    ...
                    {formData.password && passwordStrength.score < 0.6 && (
                      <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <p className="text-xs font-semibold text-yellow-800 mb-1">Suggestions:</p>
                        <ul className="text-xs text-yellow-700 space-y-1">
                          {formData.password.length < 8 && (
                            <li>• Make your password at least 8 characters long</li>
                          )}
                          {!/[A-Z]/.test(formData.password) && (
                            <li>• Add uppercase letters (A, B, C, etc.)</li>
                          )}
                          {!/[0-9]/.test(formData.password) && (
                            <li>• Include numbers (1, 2, 3, etc.)</li>
                          )}
                          {!/[^A-Za-z0-9]/.test(formData.password) && (
                            <li>• Add special characters (@, #, $, !, etc.)</li>
                          )}
                          {/(.)\1{2,}/.test(formData.password) && (
                            <li>• Avoid repeating the same character</li>
                          )}
                        </ul>
                      </div>
                    )}
