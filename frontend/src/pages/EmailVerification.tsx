...
const EmailVerification: React.FC = () => {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/auth/verify-email?token=${token}`, {
          method: 'GET',
          headers: {
      ...
      <div className="bg-white bg-opacity-60 backdrop-blur-md px-10 py-6 rounded-3xl shadow-2xl w-full max-w-lg border border-white border-opacity-50 relative z-10 text-center">
        ...
        {verificationStatus === 'success' && (
          <div className="text-center">
            ...
            <div className="text-center">
              <Link to="/" className="text-red-600 hover:underline text-sm font-semibold">
                Back to Home
              </Link>
            </div>
          </div>
        ...
        {verificationStatus === 'error' && (
          <div className="text-center">
            ...
            <div className="text-center">
              <Link to="/contact" className="text-red-600 hover:underline text-sm font-semibold">
                Contact Support
              </Link>
            </div>
          </div>
