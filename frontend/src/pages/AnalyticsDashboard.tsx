...
const AnalyticsDashboard: React.FC = () => {
  ...
  const { convertEuroToTaka, formatDualCurrency, exchangeRate } = useCurrency();
  ...
  const getCombinedStudentResults = () => {
    ...
    assessmentReports.quizzes?.forEach((quiz: any) => {
      ...
      // Extract course information from quiz data
      if (quiz.quizId?.courseId) {
        const courseInfo = {
          id: quiz.quizId.courseId._id || quiz.quizId.courseId,
          title: quiz.quizId.courseId.title || 'Course',
          level: quiz.quizId.courseId.level || 'N/A'
        };
        studentResults[studentKey].courses.add(JSON.stringify(courseInfo));
      }
    ...
    return Object.values(studentResults).map((result: any) => {
      ...
      // Get course information from enrollments or quiz data
      let courseInfo = null;
      if (studentEnrollmentInfo?.enrollments?.length > 0) {
        ...
        // Use course info from quiz data
  ...
  const fetchAssessmentReports = useCallback(async () => {
    try {
      ...
      // Fetch both assessment tests and quiz attempts
      const [testsResponse, quizAttemptsResponse] = await Promise.all([
        fetch(
          `${process.env.REACT_APP_API_URL}/tests/admin/reports?limit=50`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        ),
      ...
      setAssessmentReports({ tests: [], quizzes: [] });
    }
  }, []);
                                ...
                                <tr key={index} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${
                                  ...
                                  <td className="py-3 px-4">
                                    <div className="text-sm font-medium text-gray-900">
                                      {student.courseInfo?.title || 'Not Enrolled'}
                                    </div>
                                    {student.courseInfo?.level && (
                          ...
                          <div className="text-right">
                            ...
                            <span
                              className={`px-3 py-1 rounded-full text-sm font-medium ${
                                test.certificationLevel === "Failed"
                                  ? "bg-red-100 text-red-800"
                                  : test.certificationLevel === "C2"
                                  ? "bg-purple-100 text-purple-800"
                                  : test.certificationLevel === "C1"
                                  ? "bg-indigo-100 text-indigo-800"
                                  : test.certificationLevel === "B2"
                                  ? "bg-blue-100 text-blue-800"
                                  : test.certificationLevel === "B1"
                                  ? "bg-cyan-100 text-cyan-800"
                                  : test.certificationLevel === "A2"
                                  ? "bg-green-100 text-green-800"
                                  : "bg-yellow-100 text-yellow-800"
                              }`}
                            >
                          ...
                          <div className="bg-gray-50 p-3 rounded-lg">
                            ...
                            <p className="font-semibold">
                              {test.totalCompletionTime
                                ? `${Math.floor(
                                    test.totalCompletionTime / 60
                                  )}m ${test.totalCompletionTime % 60}s`
                                : "N/A"}
                            </p>
                          </div>
                        </div>
