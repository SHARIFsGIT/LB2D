...
const CourseVideos: React.FC = () => {
  ...
  const createUnifiedTimeline = () => {
    ...
    // Add quizzes (sort by creation date if no sequence)
    quizzes.forEach((quiz, index) => {
      timelineItems.push({
        id: quiz._id,
        type: 'quiz',
        data: quiz,
        sortOrder: 1000 + index, // Place after videos by default
        createdAt: new Date(quiz.createdAt || Date.now())
      });
    });

    // Add resources (sort by upload date if no sequence)
    resources.forEach((resource, index) => {
      timelineItems.push({
        id: resource._id,
        type: 'resource',
        data: resource,
        sortOrder: 2000 + index, // Place after quizzes by default
        createdAt: new Date(resource.uploadedAt || Date.now())
      });
    });

    // Sort by sequence/creation date
    return timelineItems.sort((a, b) => {
      // First sort by type priority (videos first, then quizzes, then resources)
      if (a.type === 'video' && b.type !== 'video') return -1;
      if (b.type === 'video' && a.type !== 'video') return 1;
      if (a.type === 'quiz' && b.type === 'resource') return -1;
      if (b.type === 'quiz' && a.type === 'resource') return 1;
      
      // Within same type, sort by sortOrder then createdAt
