...
interface UploadVideoModalProps {
  ...
  selectedCourse?: Course | null; // Optional pre-selected course
}
...
const CourseManagementModal: React.FC<CourseManagementModalProps> = ({
  ...
  const fetchQuizAttempts = async () => {
    try {
      // Fetch attempts for all quizzes in this course
      const quizzesResponse = await fetch(
        `${process.env.REACT_APP_API_URL}/quizzes/course/${course._id}`,
        {
          headers: {
      ...
      if (quizzesResponse.ok) {
        const quizzesData = await quizzesResponse.json();
        const courseQuizzes = quizzesData.data || [];
        ...
        for (const quiz of courseQuizzes) {
          try {
            const attemptsResponse = await fetch(
              `${process.env.REACT_APP_API_URL}/quizzes/${quiz._id}/attempts?status=submitted`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                  "Content-Type": "application/json",
            ...
            if (attemptsResponse.ok) {
              ...
              allAttempts.push(...(attemptsData.data || []));
            }
          } catch (error) {
            console.error(
        ...
        <div className="border-b border-gray-200">
          <nav className="flex space-x-8 px-6">
            {[
              ...
              { key: "grading", label: "Review & Grade", icon: "📊" },
            ].map((tab) => (
              <button
                key={tab.key}
                ...
                <div className="space-y-6">
                  ...
                  {quizAttempts.length === 0 ? (
                    <div className="text-center py-12 bg-gray-50 rounded-xl">
                      ...
                      <p className="text-gray-600">
                        Student quiz and exam submissions will appear here for
                        review.
                      </p>
                    </div>
                  ) : (
                      ...
                      {quizAttempts.map((attempt) => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              <h4 className="font-semibold text-gray-900">
                                {attempt.studentId?.firstName}{" "}
                                {attempt.studentId?.lastName}
                              </h4>
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <div className="flex space-x-2">
                              ...
                              >
                                {attempt.isGraded ? "Graded" : "Needs Review"}
                              </span>
                              <button
                              ...
                              >
                                {attempt.isGraded ? "View Grade" : "Grade Now"}
                              </button>
                            </div>
                          </div>
                        </div>
            ...
            <div className="flex justify-between items-start">
              <div className="flex items-center space-x-6">
                ...
                <div>
                  ...
                  <p className="text-blue-100 text-lg">{student.email}</p>
                  {student.phone && (
                    <p className="text-blue-200">📞 {student.phone}</p>
                  )}
                </div>
              </div>
              <div className="text-right">
                <div className="mb-4 flex items-center justify-end space-x-3">
                  <button
                    onClick={onClose}
                    className="text-white hover:text-pink-200 text-2xl font-bold w-8 h-8 flex items-center justify-center rounded-full hover:bg-white hover:bg-opacity-20 transition-all duration-200"
                  >
                    ×
                  </button>
                </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (uploadType === "file" && videoFile) {
        ...
        // You would call a different endpoint for file uploads
        // For now, we'll use the existing flow but note this needs backend support
        onUpload({
          ...formData,
          videoUrl: `uploaded-file://${videoFile.name}`, // Temporary indicator
          uploadType: "file",
          file: videoFile,
        });
      } else {
...
const SupervisorDashboard: React.FC = () => {
  ...
  const handleDeleteVideo = async (videoId: string) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this rejected video? This action cannot be undone."
      )
    ) {
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
                        ...
                        <div className="p-6">
                          ...
                          <div className="flex space-x-3">
                            <button
                              ...
                              className="flex-1 bg-gradient-to-r from-orange-500 to-red-500 text-white px-4 py-3 rounded-xl font-bold shadow-lg hover:shadow-xl hover:from-orange-600 hover:to-red-600 transition-all duration-300"
                            >
                              Manage Course
                            </button>
                            <button
              ...
              <div className="flex items-center justify-between">
                ...
                {(() => {
                  const totalComments = myVideos.reduce(
                    (total, video) => total + (video.commentCount || 0),
                    0
                  );
                  return (
                    totalComments > 0 && (
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs bg-white bg-opacity-30 px-2 py-1 rounded-full font-medium">
                        ...
                        <div className="mt-4">
                          {video.status === "approved" && (
                            <button
                              ...
                              className="w-full bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center justify-center space-x-2 mb-2"
                            >
                          ...
                          {video.status === "rejected" && (
                            <button
                              ...
                              className="w-full bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center justify-center space-x-2"
                            >
                              <span>🗑️</span>
      ...
      {showCourseManagementModal && selectedCourse && (
        <CourseManagementModal
          isOpen={showCourseManagementModal}
          onClose={() => {
