...
const ResourceApprovals: React.FC = () => {
  ...
  const handleApprove = async (resourceId: string) => {
    ...
    try {
      ...
      const response = await resourceApi.approve(resourceId, true);
      ...
      showError(error.message || 'Failed to approve resource');
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        showSuccess('Resource rejected successfully');
      ...
      showError(error.message || 'Failed to reject resource');
    } finally {
      setProcessingIds(prev => {
        const newSet = new Set(prev);
