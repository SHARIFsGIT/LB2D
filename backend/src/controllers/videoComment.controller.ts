...
export const addVideoComment = async (req: AuthenticatedRequest, res: Response) => {
  try {
      ...
      try {
        ...
        if (user && video && course) {
          const notificationData = {
            id: `comment_${newComment._id}_${Date.now()}`,
            type: 'video_comment',
            title: 'Student',
      ...
      try {
        ...
        if (parentComment && parentComment.userId && supervisor && video && course) {
          ...
          if (parentComment.userRole === 'Student') {
            ...
            const notificationData = {
              id: `reply_${newComment._id}_${Date.now()}`,
              type: 'video_comment',
              title: 'Supervisor',
              ...
              data: {
                ...
                studentId: studentId,
                studentName: `${parentComment.userId.firstName} ${parentComment.userId.lastName}`,
                videoId: videoId,
                videoTitle: video.title,
                courseId: courseId,
                courseTitle: course.title,
    ...
    // Send notification to supervisors when a supervisor comments (not replies) on a video
    ...
    if (userRole === 'Supervisor' && !parentCommentId) {
      try {
        ...
        if (supervisor && video && course) {
          const notificationData = {
            id: `supervisor_comment_${newComment._id}_${Date.now()}`,
            type: 'video_comment',
            title: 'Supervisor',
